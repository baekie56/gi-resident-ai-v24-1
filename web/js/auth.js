...
(function(){
 'use strict';
 let server=false,registrationCodeRequired=false,revision=0,syncTimer=null,syncing=false,dirty=false,conflict=false,activityTimer=null,activityBusy=false;
 ...
 function renderAccountLogin(mode='login'){
   ...
   if(server){
     ...
     const submit=async()=>{const name=document.getElementById('accountName').value.trim(),password=document.getElementById('accountPassword').value,button=document.getElementById('serverAccount'),registrationCode=document.getElementById('registrationCode')?.value||'';if(!name||password.length<10)return message('请输入用户名和至少10位密码');if(mode==='register'&&password!==document.getElementById('accountConfirm').value)return message('两次密码不一致');if(mode==='register'&&registrationCodeRequired&&!registrationCode)return message('请输入班级注册码');button.disabled=true;try{const j=await api(mode==='login'?'/api/login':'/api/register',{name,password,registrationCode});await beginServer(j.user)}catch(e){message(e.message);button.disabled=false}};
 ...
 window.GI24_AUTH_READY=(async()=>{
   try{if(location.protocol!=='file:'){const h=await api('/api/health');server=!!h.accounts&&h.version===GI24.VERSION;registrationCodeRequired=!!h.registrationCodeRequired;window.GI24_AI_CONFIGURED=!!h.ai}}catch(e){}
